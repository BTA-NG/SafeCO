...
- **Events**: the recent event feed with a registered-scenario selector (the
  filter is applied in the browser over the pushed window, so it narrows what is
  on screen without a second request) and ground-truth labels.
